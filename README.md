...
An interactive technical reference that maps storage across four AI workloads — training, RAG, fine-tuning, and inference — phase by phase, tier by tier, S3 path by S3 path. Updated for **GTC 2026** with the NVIDIA CMX™ context memory storage platform, the 5-tier storage model, STX rack integration, and BlueField-4 / Spectrum-X 800 GbE.
...
Four pipeline cards (Training, RAG, Fine-Tuning, Inference) with storage-intensity badges — inference now rated **active** (~55 %) since MinIO AIStor is in the inference hot path via NVIDIA CMX. Each card deep-links to its Explorer view. Bottom Line CTA links to the Comparison Matrix.
...
| View | What It Shows |
|------|---------------|
| **Reference Architecture** | Prescriptive 11-phase pipeline (incl. KV-cache overflow) — one stack, five tiers, NVIDIA CMX-aware |
| **Storage Tiers** | The 5-tier layout (T0 NVMe Block, **Tier G3.5 NVIDIA CMX KV Overflow**, T1 Hot S3, T2 S3 over RDMA, T3 Cold Archive) with Data Lake vs Lakehouse modal |
...
| **Inference** | Model serving — NVIDIA CMX G3.5 KV-cache overflow, continuous sub-ms RDMA I/O during generation |
...
- **Common Misconceptions** — five myths about AI storage, busted with technical reality (Myth #1 now qualified for NVIDIA CMX)
- **Glossary Terms** — 55+ terms across three categories (AI/ML, Storage Infrastructure, MinIO AIStor), searchable, filterable by category, with related-term navigation. Includes GTC 2026 terms: NVIDIA CMX™, Tier G3.5, STX, BlueField-4, Spectrum-X 800 GbE, ConnectX-9, NIXL, Grove, Dynamo, Vera Rubin, GPUDirect RDMA for S3.
...
| **G3.5** | **NVIDIA CMX KV-Cache Overflow** | GPUDirect RDMA for S3 (800 GbE Spectrum-X) | < 1 ms | **Yes** |
...
| NVIDIA CMX inference boost | **5× tokens/sec** | vs KV eviction/recompute |
| NVIDIA CMX power efficiency | **5× power efficiency** | vs KV eviction/recompute |
| NVIDIA CMX KV overflow latency | **< 1 ms** | RDMA over 800 GbE |
...
src/
  components/
    ...
    ReferenceArchitecture.tsx          # 11-phase prescriptive guide (5-tier, NVIDIA CMX-aware)
    StorageLayoutExplorer.tsx          # 5-tier storage visualization (incl. NVIDIA CMX G3.5)

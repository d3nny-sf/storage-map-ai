...
const nodes: NodeData[] = [
  {
    ...
    minioFeature: 'MinIO AIStor: ~34.4 GB/s PUT on an 8-node reference cluster, scaling linearly with node count. Disaggregated architecture enables independent compute/storage scaling',
    dataVolume: 'PB',
    phase: 1,
  },
  {
    ...
    minioFeature: 'S3 Select with SIMD acceleration — server-side filtering on CSV/JSON/Parquet, 80%+ bandwidth reduction (whitepaper: SIMD-accelerated S3 Select)',
    dataVolume: 'TB',
    phase: 1,
  },
  {
    ...
    ioProfile: {
      ...
      throughput: '~103.5 GB/s aggregate GET (8-node reference)',
  ...
  },
  {
    ...
    minioFeature: 'MinIO Cache: distributed shared DRAM cache prevents GPU starvation. 8-node reference cluster sustains ~103.5 GB/s aggregate GET (~12.9 GB/s/node), scaling with nodes',
    dataVolume: 'GB/s',
    phase: 2,
  },
  {
  ...
  },
  {
    ...
    minioFeature: 'Reed-Solomon erasure coding (12-drive, 6-parity tolerates 5 failures); BitRot via HighwayHash >10 GB/s/core (whitepaper: per-object inline EC in assembly)',
    dataVolume: 'TB',
    phase: 4,
  },
  {
  ...
  },
  {
    ...
    minioFeature: 'Object Lock (WORM): SEC 17a-4(f), FINRA 4511(c) compliant immutable retention (whitepaper: retention + legal hold + governance mode)',
    dataVolume: 'GB',
    phase: 5,
  },
]
            ...
            <button
              ...
              className={`flex-1 py-2 px-3 rounded-lg text-xs font-medium transition-all duration-300 ${
                currentPhase === phase.id
                  ? 'bg-raspberry text-white shadow-lg shadow-raspberry/30 scale-[1.03]'
                  : currentPhase > phase.id

...
const roleColors = {
  ...
  'active-tier': { fill: '#0891B2', stroke: '#22D3EE', text: 'text-cyan-500', bg: 'bg-gradient-to-r from-teal-500 to-cyan-600', glow: 'rgba(8, 145, 178, 0.4)' },
}
  ...
  return (
    <div className="bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900 rounded-2xl overflow-hidden shadow-2xl border border-white/10">
      ...
      <div className="relative p-6 overflow-x-auto" style={{ minHeight: '880px' }}>
        <svg viewBox="0 0 1120 860" className="w-full min-w-[1000px]">
          <defs>
            ...
            <linearGradient id="infCmxGrad" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stopColor="#164E63" /><stop offset="100%" stopColor="#0E7490" /></linearGradient>
            ...
            <marker id="infArrowCyan" markerWidth="22" markerHeight="16" refX="20" refY="8" orient="auto" markerUnits="userSpaceOnUse"><polygon points="0 0, 22 8, 0 16" fill="#0891B2" /></marker>
          ...
          {flowPaths.map(flow => {
            ...
            const markerKey = fromNode.role === 'primary' ? 'Primary' : fromNode.role === 'burst' ? 'Burst' : fromNode.role === 'not-in-path' ? 'Green' : fromNode.role === 'active-tier' ? 'Cyan' : 'Cyan'
            return (
          ...
          {nodes.filter(n => n.id !== 'inference-engine').map(node => {
            ...
            return (
              <g key={node.id} className="cursor-pointer" style={{ opacity: isActive ? 1 : 0.4 }}
                ...
                <rect x={node.x} y={node.y} width={node.width} height={node.height} rx="12" fill={`url(#${gradientId})`} stroke={colors.fill} strokeWidth={isSelected ? 3 : 2} />

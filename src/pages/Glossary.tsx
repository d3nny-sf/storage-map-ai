...
const glossaryTerms: GlossaryTerm[] = [
  ...
  },
  {
    ...
    context: 'A 70B model (140GB) on an 8-node AIStor reference cluster (~103.5 GB/s aggregate GET) loads in roughly a second. At 10 GB/s, ~14 seconds. At 1 GB/s, over 2 minutes. Throughput scales with node count.',
  ...
  },
  {
    ...
    context: 'Modern DataLoaders use prefetch buffers and multiple workers to hide storage latency. The goal: enough aggregate read throughput to keep GPU clusters fed — an 8-node AIStor reference cluster sustains ~103.5 GB/s aggregate GET, scaling linearly with nodes.',
  ...
  },
  {
    ...
    definition: 'A data architecture pattern that organizes data into three layers: Bronze (raw), Silver (cleaned/validated), and Gold (curated/ready for consumption). Used with Iceberg/Delta AIStor Tables on MinIO AIStor.',
  ...
  },
  {
    ...
    context: 'WebDataset enables sequential reads and easy shuffling at the shard level — ideal for distributed training from S3 at the high aggregate throughput AIStor reference clusters deliver (~103.5 GB/s GET on 8 nodes, scaling with node count).',
  ...
  // =========================================================================
  {
    ...
    definition: 'NVIDIA\'s DPU (Data Processing Unit) family of SuperNICs. Offloads networking, security, and storage acceleration off the host CPU, and provides the RDMA data path used by the G3.5 context-memory layer.',
    context: 'DPUs keep the CPU out of the data path for RDMA transfers — the same principle MinIO MemKV relies on at the G3.5 layer. Combined with Spectrum-X 800 GbE, this delivers the low-latency fabric inference workloads need.',
    relatedTerms: ['STX Rack', 'G3.5 Layer (MinIO MemKV)', 'Spectrum-X 800 GbE', 'RDMA'],
    category: 'storage',
  },
  {
    term: 'MinIO MemKV (Context Memory)',
    definition: 'MinIO MemKV is a context-memory product purpose-built for the G3.5 layer of the GPU memory hierarchy — the tier between GPU HBM and object storage. KV cache moves from GPU memory to shared NVMe over RDMA, bypassing the file system and object protocols entirely, with no CPU in the data path. It runs inside NVIDIA STX as a single ARM64-native binary embedded in the storage tier.',
    context: 'Operates in 2–16 MB blocks tuned for GPU access patterns (not legacy 4 KB), over Spectrum-X 800 GbE and PCIe Gen6 at near wire speed. Sustains 95%+ GPU utilization cluster-wide and lets long-context and agentic workloads run at 100× scale with consistent latency. GPU compute and context memory scale independently.',
    relatedTerms: ['KV Cache', 'G3.5 Layer (MinIO MemKV)', 'RDMA', 'TTFT'],
    category: 'storage',
  },
  {
    term: 'ConnectX-9',
    definition: 'NVIDIA\'s network adapter family with hardware RDMA offload at 800 GbE, providing the low-latency fabric for context-memory transfers and GPU-to-GPU gradient sync.',
    context: 'Part of the Spectrum-X 800 GbE platform. The RDMA offload is what lets the G3.5 layer move KV cache without a CPU in the path.',
  ...
  },
  {
    ...
    context: 'In the storage-agnostic ILM lens, Tier 1 uses S3 over RDMA to 100% NVMe. In the NVIDIA memory-hierarchy lens, the G3.5 context-memory layer uses RDMA to move KV cache GPU→NVMe with no CPU in the data path. Reference point: an 8-node AIStor cluster sustains ~103.5 GB/s aggregate GET (~12.9 GB/s per node).',
    relatedTerms: ['RoCE', 'G3.5 Layer (MinIO MemKV)', 'Spectrum-X 800 GbE', 'ConnectX-9'],
    category: 'storage',
  },
  {
    term: 'S3 API',
  ...
  {
    term: 'Dynamo (NVIDIA)',
    definition: 'NVIDIA\'s inference orchestration framework. Coordinates request routing, KV-cache placement, and model-replica scaling across GPU fleets, working with NIXL for zero-copy transfers.',
    context: 'Dynamo decides which replica serves a request and where multi-turn context lives. The G3.5 context-memory layer (MinIO MemKV) is where that KV state is held so it does not have to be recomputed.',
    relatedTerms: ['NIXL', 'G3.5 Layer (MinIO MemKV)', 'vLLM', 'Inference'],
    category: 'storage',
  },
  {
    term: 'GPUDirect RDMA for S3',
    definition: 'A technology that enables zero-copy data transfers from S3 object storage directly into GPU memory over RDMA, bypassing CPU and system memory entirely. Announced as production-standard at GTC 2026.',
    context: 'Enables direct GPU ← S3 reads for model loading at line rate (800 GbE) with MinIO AIStor as the object-storage (G4) layer. The G3.5 context-memory layer (MinIO MemKV) uses the same RDMA fabric.',
    relatedTerms: ['GPU-Direct Storage (GDS)', 'RDMA', 'BlueField-4 SuperNIC', 'MinIO AIStor'],
    category: 'storage',
  },
  {
    term: 'Grove (NVIDIA)',
    ...
    context: 'Grove manages what stays in VRAM vs. what lives at the G3.5 context-memory layer. Works with Dynamo for cross-replica cache sharing; MinIO MemKV is the product purpose-built for that G3.5 layer.',
    relatedTerms: ['KV Cache', 'G3.5 Layer (MinIO MemKV)', 'Dynamo (NVIDIA)'],
    category: 'storage',
  },
  {
    term: 'NIXL',
    ...
    context: 'NIXL is the transfer plumbing underneath Dynamo. It handles the RDMA moves that place KV cache at the G3.5 context-memory layer, ensuring microsecond latency with no CPU in the path.',
    relatedTerms: ['Dynamo (NVIDIA)', 'G3.5 Layer (MinIO MemKV)', 'GPUDirect RDMA for S3'],
    category: 'storage',
  },
  {
    ...
    context: 'The networking backbone for the G3.5 context-memory layer, GPU-to-GPU gradient sync, and MinIO AIStor data-plane traffic — all over RDMA (RoCE v2).',
    relatedTerms: ['ConnectX-9', 'RDMA', 'STX Rack', 'G3.5 Layer (MinIO MemKV)'],
    category: 'storage',
  },
  {
    term: 'STX Rack',
    definition: 'NVIDIA\'s liquid-cooled, integrated AI rack system (GTC 2026). Converges Vera Rubin GPUs, BlueField-4 SuperNICs, NVMe storage, and Spectrum-X 800 GbE networking into a single rack-scale unit.',
    context: 'NVIDIA STX is where MinIO MemKV runs — a single ARM64-native binary embedded in the storage tier, co-located with GPUs for microsecond RDMA. Think "converged infrastructure for AI" — not disaggregated.',
    relatedTerms: ['Vera Rubin', 'BlueField-4 SuperNIC', 'Spectrum-X 800 GbE', 'G3.5 Layer (MinIO MemKV)'],
    category: 'storage',
  },
  {
    term: 'G3.5 Layer (MinIO MemKV)',
    definition: 'The layer of the GPU memory hierarchy between GPU HBM (G3) and networked object storage (G4) — a shared, flash-backed KV pool over RDMA. MinIO MemKV is purpose-built for it: KV cache moves from GPU memory to NVMe with no file system, no object protocol, and no CPU in the data path.',
    context: 'When context length exceeds HBM, the KV cache lives here instead of being evicted and recomputed. A petascale pool deployed across the inference cluster, 2–16 MB blocks, sustaining 95%+ GPU utilization cluster-wide. This is where context memory enters the inference path.',
    relatedTerms: ['MinIO MemKV (Context Memory)', 'KV Cache', 'RDMA', 'STX Rack'],
    category: 'storage',
  },
  {
    term: 'Vera Rubin (NVIDIA GPU)',
    definition: 'NVIDIA\'s next-generation GPU architecture announced at GTC 2026, succeeding Blackwell. Rubin Ultra is the top SKU. Features next-gen HBM and improved FP4/FP6 compute.',
    context: 'Vera Rubin is the reference GPU in the prescriptive stack. Paired with BlueField-4, ConnectX-9, and Spectrum-X 800 GbE in the STX rack.',
    relatedTerms: ['STX Rack', 'BlueField-4 SuperNIC', 'G3.5 Layer (MinIO MemKV)'],
    category: 'storage',
  },
  ...
  // =========================================================================
  {
    ...
    context: 'The whitepaper positions AIStor as the complete storage platform for AI: from PB-scale data lakes to model registries. Reference benchmark: an 8-node cluster sustains ~103.5 GB/s aggregate GET (~12.9 GB/s per node) with 4.4 PB usable, scaling linearly as nodes are added.',
  ...
  {
    term: 'AIStor Tables (S3 Tables / Iceberg)',
    definition: 'Open table formats (Apache Iceberg, Delta Lake, Apache Hudi) providing ACID transactions, time travel, and schema evolution on top of S3 object storage. AIStor Tables is MinIO\'s native Iceberg integration — query-in-place without data movement.',
    context: 'The Medallion architecture (Bronze → Silver → Gold) runs on AIStor Tables. MinIO provides the S3 foundation; Iceberg provides the table semantics. Announced at GTC 2026 as a first-class feature.',
    relatedTerms: ['Medallion Architecture', 'Iceberg', 'Data Lake', 'MinIO AIStor'],
    category: 'minio',
  },
  {
...
const myths = [
  {
    ...
    reality: 'For short-context inference, everything happens in GPU memory — the model is loaded, the KV cache lives in VRAM, and storage only touches model load + logging. HOWEVER, long-context and agentic workloads blow past HBM, and the KV cache has to live somewhere. That is the G3.5 layer of the GPU memory hierarchy, and MinIO MemKV is purpose-built for it: KV cache moves from GPU memory to shared NVMe over RDMA — no file system, no object protocol, no CPU in the path — so GPUs decode instead of recomputing context. For those workloads, a memory tier IS in the inference path.',
  },
  {
